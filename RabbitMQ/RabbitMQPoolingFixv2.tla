...
begin RC:
    while TRUE do
        ...
        Clear_On_Recovery:
            channel.PendingTasks := {} || channel.Recovered := TRUE
        end if;
    end while;
end process;
...
begin P:
  while TRUE do
    ...
    Clean_The_Channel_On_Get:
     if channel.Recovered = TRUE then
        channel.PendingTasks := {} || channel.Recovered := FALSE;
     end if;
...
Get_Channel == /\ pc[1] = "Get_Channel"
               ...
               /\ pc' = [pc EXCEPT ![1] = "Clean_The_Channel_On_Get"]
               /\ tcs' = tcs

Clean_The_Channel_On_Get == /\ pc[1] = "Clean_The_Channel_On_Get"
                            /\ IF channel.Recovered = TRUE
                                  THEN /\ channel' = [channel EXCEPT !.PendingTasks = {},
                                                                     !.Recovered = FALSE]
                                  ELSE /\ TRUE
                                       /\ UNCHANGED channel
...
Release_Channel == /\ pc[1] = "Release_Channel"
                   /\ IF channel.Client.IsOpen = FALSE
                         THEN /\ channel' = [channel EXCEPT !.Client = [IsOpen |-> FALSE, NextSequenceNo |-> 0],
                                                            ...
                                                            !.InPool = FALSE]
                              /\ pc' = [pc EXCEPT ![1] = "P"]
                         ELSE /\ pc' = [pc EXCEPT ![1] = "Clean_The_Channel_On_Receive"]
                              /\ UNCHANGED channel
                   /\ tcs' = tcs

Clean_The_Channel_On_Receive == /\ pc[1] = "Clean_The_Channel_On_Receive"
                                /\ IF channel.Recovered = TRUE
                                      THEN /\ channel' = [channel EXCEPT !.PendingTasks = {},
                                                                         !.Recovered = FALSE]
                                      ELSE /\ TRUE
                                           /\ UNCHANGED channel
                                /\ pc' = [pc EXCEPT ![1] = "Receive_After_Clean"]
                                /\ tcs' = tcs

Receive_After_Clean == /\ pc[1] = "Receive_After_Clean"
                       /\ channel' = [channel EXCEPT !.InPool = TRUE]
                       /\ pc' = [pc EXCEPT ![1] = "P"]
                       /\ tcs' = tcs

Transport == P \/ Get_Channel \/ Clean_The_Channel_On_Get \/ Create_Tcs
                \/ Store_Tcs \/ Send_Message \/ Release_Channel
                \/ Clean_The_Channel_On_Receive \/ Receive_After_Clean
...
\* Last modified Mon Sep 10 11:21:16 CEST 2018 by Tomasz Masternak
\* Created Mon Sep 10 11:10:06 CEST 2018 by Tomasz Masternak
